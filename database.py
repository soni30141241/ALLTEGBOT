...
class Database:
    ...
    def get_members(self, chat_id):
        with self.conn() as c:
            return c.execute(
                "SELECT user_id,name FROM members WHERE chat_id=?",
                (chat_id,),
            ).fetchall()
    ...
    def get_settings(self, chat_id):
        with self.conn() as c:
            row = c.execute(
                "SELECT anti_spam,abuse_filter,anti_food,warn_limit "
            ...
            return dict(
                zip(
                    ["anti_spam", "abuse_filter", "anti_food", "warn_limit"],
                    row,
                )
            )
    ...
    def set_warn_limit(self, chat_id, limit):
        ...
        with self.conn() as c:
            c.execute(
                "UPDATE settings SET warn_limit=? WHERE chat_id=?",
                (limit, chat_id),
            )
